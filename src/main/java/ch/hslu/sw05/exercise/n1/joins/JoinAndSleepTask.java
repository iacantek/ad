...
public class JoinAndSleepTask implements Runnable {
    ...
    private Thread joinFor = null;

    /**
     * Erzeugt einen Task mit Namen und Dauer.
     * 
    ...
    @Override
    public void run() {
        LOG.info(this.taskName + " started... (" + Thread.currentThread().getName() + ")");
        if (joinFor != null) {
            try {
                LOG.info(taskName + " waits for " + joinFor.getName());
                joinFor.join();
            } catch (InterruptedException ex) {
                LOG.error(ex.getMessage());
            }
            LOG.info(taskName + " has awakened");
        }

        try {
            LOG.info(taskName + " sleeps for " + sleepTime + "ms");
            Thread.sleep(sleepTime);
            LOG.info(taskName + " finished sleeping");
